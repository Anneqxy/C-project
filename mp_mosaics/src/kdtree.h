...
class KDTree
{
  ...
  public:
    ...
    void destroy(typename KDTree<Dim>::KDTreeNode* currentNode);
    KDTreeNode* copyTree(typename KDTree<Dim>::KDTreeNode* otherNode);
    KDTreeNode* KDTree_helper(vector<Point<Dim>>& input, int left, int right, int dim);
    // void constructKDTree(const vector<Point<Dim>>& newPoints, int dim, KDTreeNode*& root);
    ...
    Point<Dim> findNearestNeighborHelper(const Point<Dim> &query, KDTreeNode *current, int currentDimension) const;
    Point<Dim> findNearestNeighborRecursive(const Point<Dim> &query, KDTreeNode *current, int currentDimension) const;
    double euclideanDistance(const Point<Dim> &a, const Point<Dim> &b) const;
...
// template <typename RandIter, typename Comparator>
// RandIter select_helper(RandIter start, RandIter end, Comparator cmp);

// template <int Dim>
// void destroy(typename KDTree<Dim>::KDTreeNode* currentNode);

// template <int Dim>
// typename KDTree<Dim>::KDTreeNode* copyTree(typename KDTree<Dim>::KDTreeNode* otherNode);

// template <int Dim, typename Comparator>
// typename KDTree<Dim>::KDTreeNode* KDTree_helper(vector<Point<Dim>>& input, int left, int right, int dim);

...
void FloodFilledImage::addFloodFill(Traversals::ImageTraversal & traversal, ColorPicker & colorPicker) {
  /** @todo [Part 2] */
  vector.push_back(&traversal);
  vector2.push_back(&colorPicker);
}
...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  ...
  Animation animation;
  PNG tem = *png_;
  animation.addFrame(tem);
  // int frame = 0;

  for (unsigned int i = 0; i < vector.size(); i++) {
    Traversals::ImageTraversal* traversal=vector[i];
    ColorPicker* color = vector2[i];
    int frame = 0;
    for (auto it = traversal->begin(); it != traversal->end(); ++it) {
      Point p = *it;
      tem.getPixel(p.x, p.y) = color->getColor(p.x, p.y);
      frame++;
